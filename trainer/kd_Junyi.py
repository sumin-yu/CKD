...
from trainer.loss_utils import compute_Junyi_loss
import trainer
...
class Trainer(trainer.GenericTrainer):
    ...
    def _train_epoch(self, epoch, train_loader, model, teacher, distiller=None):
        ...
        for i, data in enumerate(train_loader):
            ...
            kd_loss = compute_Junyi_loss(outputs, t_outputs, kd_temp=self.kd_temp, device=self.device)

            # loss = self.criterion(outputs, labels) + self.lambh * kd_loss
            loss = (1 - self.alpha_J) * self.criterion(outputs, labels) + self.alpha_J * kd_loss
