    ...
    def get_statistics(self, dataset, bs=128, n_workers=2, model=None):
        ...
        with torch.no_grad():
            for i, data in enumerate(dataloader):
                inputs, _, sen_attrs, targets, filter_indicator = data
                batch_size = inputs.shape[0]
                inputs = inputs.permute((1,0,2,3,4))
                inputs = inputs.contiguous().view(-1, *inputs.shape[2:])
                
                sen_attrs = torch.reshape(sen_attrs.permute((1,0)), (-1,))
                targets = torch.reshape(targets.permute((1,0)), (-1,)).type(torch.LongTensor)
                
                org_filtered_idx = torch.arange(batch_size)

                y_set.append(targets[org_filtered_idx]) # sen_attrs = -1 means no supervision for sensitive group
                s_set.append(sen_attrs[org_filtered_idx])
